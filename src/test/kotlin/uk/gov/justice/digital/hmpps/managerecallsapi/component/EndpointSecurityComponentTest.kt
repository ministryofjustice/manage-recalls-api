...
class EndpointSecurityComponentTest : ComponentTestBase() {
  ...
  private fun roleManageRecallsRequiredRequestBodySpecs(): Stream<WebTestClient.RequestHeadersSpec<*>>? {
    return Stream.of(
      ...
      webTestClient.post().uri("/recalls/${UUID.randomUUID()}/documents").bodyValue(uploadDocumentRequest),
      ...
      webTestClient.patch().uri("/recalls/${UUID.randomUUID()}/documents/create").bodyValue(generateDocumentRequest),
