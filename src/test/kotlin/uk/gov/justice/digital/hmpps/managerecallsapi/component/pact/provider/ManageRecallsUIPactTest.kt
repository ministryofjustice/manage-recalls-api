...
class ManagerRecallsUiAuthorizedPactTest : ManagerRecallsUiPactTestBase() {
  ...
  fun `a document does not exist`() {
    every {
      documentService.updateDocumentCategory(any(), any(), any())
  ...
  fun `a letter to prison exists`() {
    ...
    every { documentService.getVersionedDocumentContentWithCategoryIfExists(any(), LETTER_TO_PRISON) } returns letterBytes
  }
