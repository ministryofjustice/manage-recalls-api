...
class ManagerRecallsUiAuthorizedPactTest : ManagerRecallsUiPactTestBase() {
  ...
  fun `a recall notification and recall exists`() {
    `a user and a fully populated recall without documents exists`()
    `a document exists`(::DocumentId.random(), RECALL_NOTIFICATION, 2)
  }
  ...
  fun `a dossier and recall exists`() {
    `a user and a fully populated recall without documents exists`()
    `a document exists`(::DocumentId.random(), DOSSIER, 1)
  }
  ...
  fun `a letter to prison and recall exists`() {
    `a user and a fully populated recall without documents exists`()
    `a document exists`(::DocumentId.random(), LETTER_TO_PRISON, 1)
  }
  ...
  fun `a document exists`(documentId: DocumentId, documentCategory: DocumentCategory, version: Int?) {
    val documentBytes =
