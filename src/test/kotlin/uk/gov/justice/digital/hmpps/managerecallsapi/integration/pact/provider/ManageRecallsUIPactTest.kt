...
class ManagerRecallsUiAuthorizedPactTest : ManagerRecallsUiPactTestBase() {
  ...
  fun `a document can be created`() {
    val recallId = RecallId(UUID.fromString("00000000-0000-0000-0000-000000000000"))
    val documentBytes = "Hello".toByteArray()
    ...
    every {
      recallDocumentService.addDocumentToRecall(recallId, documentBytes, category)
    } returns documentId
  ...
  fun `a document exists`() {
    ...
    every { recallDocumentService.getDocument(recallId, documentId) } returns
      Pair(
